...
<?php
		...
		if($keywords){
			...
			while (list($name, $ops) = each($keywords)) {
				...
				$times = intval($ops['times']);
				if($times == 0){
					$times = '';
				}

				$between = intval($ops['between']);
				if($between == 0){
					$between = '';
				}

				$before = intval($ops['before']);
				if($before == 0){
					$before = '';
				}

				$after = intval($ops['after']);
				if($after == 0){
					$after = '';
				}
				...
				echo "
					<tr>
						...
						<td><input type='text' style='width:90%;' value='$url' name='internal_link_building[$x][url]' /></td>
						<td><input type='text' value='$times' name='internal_link_building[$x][times]' size='4' /></td>
						<td><input type='text' value='$between' name='internal_link_building[$x][between]' size='4' /></td>
						<td><input type='text' value='$before' name='internal_link_building[$x][before]' size='4' /></td>
						<td><input type='text' value='$after' name='internal_link_building[$x][after]' size='4' /></td>
...
global $wpdb;
	...
	$max_keys = intval($options['max_keys']);
	if($max_keys == 0){
		$max_keys = '';
	}
				...
				<div class="dbx-b-ox-wrapper">
					...
					<fieldset id="accesskeys" class="dbx-box">
						<div class="dbx-h-andle-wrapper"><h3 class="dbx-handle">General Options </h3></div>
							<div class="dbx-c-ontent-wrapper">
								<div class="dbx-content">
								<p>Maximimum number of keywords in a single post? <input type="text" value="<?php echo $max_keys ?>" name="internal_link_building_options[max_keys]" /> (Leave this blank for unlimited)</p>
